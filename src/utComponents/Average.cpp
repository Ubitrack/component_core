...
protected:
	...
	typename ResultType::value_type mean( const EventList &eList )
	{
		typename ResultType::value_type tmp = typename ResultType::value_type();
		int size = eList.size();
	...
	{
		EventList eventList = *m_inPort.get();
		typename ResultType::value_type tmp = mean( eventList );
...
template<>
Math::Vector< 3 > Average< Measurement::Position, Measurement::Position >::mean( const std::vector< Math::Vector< 3 > > &eList )
{
	int size = eList.size();
...
template<>
Math::ErrorVector< 3 > Average< Measurement::Position, Measurement::ErrorPosition >::mean( const std::vector< Math::Vector< 3 > > &eList )
{
	int size = eList.size();
...
template<>
Math::Quaternion Average< Measurement::Rotation, Measurement::Rotation >::mean( const std::vector< Math::Quaternion > &eList )
{
	unsigned size = eList.size();
...
template<>
Math::Pose Average< Measurement::Pose, Measurement::Pose >::mean( const std::vector< Math::Pose > &eList )
{
	unsigned size = eList.size();
...
template<>
Math::ErrorPose Average< Measurement::Pose, Measurement::ErrorPose >::mean( const std::vector< Math::Pose > &eList )
{
	unsigned size = eList.size();
